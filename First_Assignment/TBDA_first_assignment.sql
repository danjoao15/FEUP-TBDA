...
create table z_xucs as select * from GTD10.xucs;
  ...
  alter table z_xdocentes add constraint zdocentes_pk primary key (nr);
  alter table z_xdsd add constraint zdsd_pk primary key (nr,id);
  alter table z_xtiposaula add constraint ztiposaula_pk primary key (id);
  alter table z_xocorrencias add constraint zocorrencias_pk primary key (codigo,ano_letivo,periodo);
  alter table z_xucs add constraint zucs_pk primary key (codigo);

  alter table z_XDSD add constraint ZDOCENTES_fk foreign key (nr) references z_XDOCENTES(nr);
  alter table z_XDSD add constraint ZTIPOSAULA_fk foreign key (id) references z_XTIPOSAULA(id);
  alter table z_XTIPOSAULA add constraint ZOCORRENCIAS_fk foreign key (CODIGO,ANO_LETIVO,PERIODO) references z_XOCORRENCIAS(CODIGO,ANO_LETIVO,PERIODO);
  alter table z_XOCORRENCIAS add constraint ZUCS_fk foreign key (CODIGO) references z_XUCS(CODIGO);
...
--Question 6
CREATE INDEX index_codigo_tipo ON z_xtiposaula(codigo, tipo);
